...
from method.data_process import Dataset4MS_SL,VisDataSet4MS_SL,\
    TxtDataSet4MS_SL,collate_train,read_video_ids
from method.eval_dtt import eval_epoch,start_inference
...
def train_epoch(model, train_loader, optimizer, opt, epoch_i, training=True):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader), desc="Training Iteration", total=num_training_examples):
        global_step = epoch_i * num_training_examples + batch_idx #整体上来看，是第几个batch
        dataloading_time.update(round(time.time() - timer_dataloading),4)
        ...
        model_forward_time.update(round(time.time() - timer_start,4))
        timer_start = time.time()
        if training:
            optimizer.zero_grad()
            loss.backward()
            ...
            optimizer.step()
            model_backward_time.update(round(time.time() - timer_start,4))
...
def start_training(opt):
    ...
    # train_dataset = Dataset4MS_SL(opt.root_path,opt.visual_feature,opt.collection, opt)

    # val_text_dataset = TxtDataSet4MS_SL(opt.root_path,opt.visual_feature,opt.collection, opt,type_of_dataset= 'val')

    # val_video_ids_list = read_video_ids(caption_files['val'])
    # val_video_dataset = VisDataSet4MS_SL(opt.root_path,opt.visual_feature,opt.collection, opt, video_ids=val_video_ids_list)

    train_dataset = Dataset4MS_SL(opt)

    val_text_dataset = TxtDataSet4MS_SL(opt,type_of_dataset= 'val')
