...
class BaseOptions(object):
    ...
    tensorboard_log_dir = "tensorboard_log"# 存放tensorboard的地方
    ...
    def display_save(self, opt):
        ...
        if not isinstance(self, TestOptions):
            option_file_path = os.path.join(opt.results_dir, self.saved_option_filename)  # not yaml file indeed  这是一个json文件
    ...
    def parse(self):
        ...
        #print("1111111111111111111111111111111122222222222222222222222222233333333333333333333333344444444444444444445555555555555555556666666666666666666666",opt.root_path)

        if opt.debug:
            opt.results_root = os.path.sep.join(opt.results_root.split(os.path.sep)[:-1] + ["debug_results", ])##  认为把result_root的result去掉，换成debug_results
            opt.no_core_driver = True
            opt.num_workers = 0
            opt.eval_query_bsz = 100
        if isinstance(self, TestOptions):
            # modify model_dir to absolute path
            opt.model_dir = os.path.join(opt.root_path, opt.collection,"results",opt.model_dir)# root_path是dataset的地方,colelection是dataset下面的三种数据集，model_dir是model.ckpt所在的陌路
